...
Diese Einbettung wurde im Zuge dieser Diplomarbeit erweitert. Nun ist es  möglich Scala-Funktionen und -Werte in einem gewissen Rahmen automatisch zu übersetzen und typsicher im \ac{SL} Code zu benutzen.
...
%TODO übersicht über die Kapitel
...
%TODO das typsystem ist nicht einfache!!!
...
%TODO option nach translate_type verschieben
...
%nochmal darauf verweisen welche primitive mit welchen primitiven assoziert sind | siehe translate_funktion dabei entsteht das problem der unterschiedlichen werte bereiche.
...
Insbesondere bei den ganzzahligen Primitiven ist das Problem unumgänglich.  Für ihre Wertebereiche gilt:
\begin{center}
...
\end{table}
\footnotetext[2]{Alle Zahlendatentypen werden in \ac{JS} durch den primitiven Number Datentyp dargestellt. Dies ist eine Gleitkommazahldarstellung nach dem IEEE 754 Standard mit einer Breite von 64 Bit. In dieser Darstellung können Ganzzahlwerte von $-2^{53} + 1$ bis $2^{53} -1$ korrekt dargestellt werde.}
...
%TODO formalisierte übersetzung von option und eine nicht formalisierte übersetzung anhand des beispiels seq
...
%TODO translate_type genauer erklären Seq[AbstractTranslator] ist Teilmenge von translate_type.

%TODO erklären das man wenn man neue Typen übersetzen möchte neue Translatorklassen schreiben muss.

%TODO im anhang Beispielhaft einen neuen Typ übersetzen
...
%TODO implementation von OptionTranslator beschreiben
...
%TODO irgendwo muss noch hin wie funktionen und werte jetzt benutzt werden
...
Es blieb aber ein grundsätzliches Problem erhalten. Wie kann der generierte \ac{JS}-Code auf das Serverumfeld wie Datenbanken, Session oder Benutzerdaten zugreifen. In herkömmlichen Anwendungen gibt es zwei Lösungen dafür: Entweder man bindet die Daten direkt in den Quellcode der einzelnen Webseiten ein oder lädt sie mit Hilfe von asynchronen Anfragen nach. 
%TODO den satz neu schreiben was macht man damit genau
In der aktuellen Version von \ac{SL} kann man Scala-Werte direkt im \ac{SL} Code benutzen und Daten über übersetzte Scala Funktionen nachladen bzw. verändern.

%TODO überschrift überarbeiten welches projekt
...
%TODO was macht das macro genau (Schrittweise)
...
%TODO erklären warum
 ...
 \item[-]{Die Funktion darf nur eine Parameterliste haben\footnote{In der aktuellen Implementation werden die Default-Werte eines Parameters ignoriert. Eine entsprechende warning wird erzeugt.}}
...
%TODO erwähnen das ich nicht weis wie scala_to_sl und andersherum heist.h
