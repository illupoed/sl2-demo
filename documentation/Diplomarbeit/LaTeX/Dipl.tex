...
% - webprogrammierung nach mvc modell
% - in dieser arbeit geht es um die einbettung des typsystems von scala in js
% -> die in dieser arbeit vorgestellten konzepte lassen sich nicht so einfach auf andere sprachen ummünzen ->
% -> es sollen projekte betrachtet die auch js in scala einbetten
% - js code mithilfe des scala compilers erzeugen
% - soll typsicherheit nach js bringen
%
% - können sie scala typen in js übertragen <- also können werte aus scala im js code genutzt werden
% - ist es möglich den geschriebenen models zu sharen <- also auch funktionen/methoden
% - wie gut lassen sich existierende js-bibliotheken einbinden
% - wie schnell ist das ganze
% - welchen aufwand haben die ansätze während der laufzeit
% - im laufe der betrachtungen werden noch weitere feinheiten betrachtet
% - können bibliotheken erzeugt werden

% - jscala fällt raus <- bringt keien typsicherheit mit
